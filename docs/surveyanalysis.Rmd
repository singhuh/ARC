...
datatable(head(survy), options = list(pageLength=50, scrollX='400px'), filter = 'top')
```
...
survy %>% tbl_summary(by = age_group,
                      include = c(Q1, Q2, Q3, Q4, Q5, Q6, Q7, Q8),
                      label = list(Q1 = "Q1. Do you know what Hemoglobin A1C testing is?",
                                   ...
                                   Q5 = "Q5. I scheduled an appointment to give so that I could receive a complimentary A1C test.",
                                   Q6 = "Q6. Knowing about complimentary A1C testing was a factor in my decision to donate blood in March.",
...
##### Q4
```{r class.source = 'fold-hide', warning=FALSE,  message=FALSE, results='show',fig.width=12, fig.height=6, out.width="100%", out.height="60%"}
survy$Q4 <- factor(survy$Q4, levels = c("Strongly Agree", "Agree", "Neutral", "Disagree", "Strongly Disagree"))
df <- survy %>% filter(Gender != "") %>%
...
p +  geom_text(aes(label=paste0(counts,"(", 100*percent,"%)"), 
  ...
  guides(color = guide_legend(title = "New Species Title"))+
  # scale_y_discrete(breaks = NULL) +
  ggtitle("Q4. Learning my A1C level through a blood donation is of interest to me.")+ theme(legend.position = "top")
